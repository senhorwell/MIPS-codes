...
maior: .asciiz "\nMaior temperatura: "
media: .asciiz "\nTemperatura média: "
menormedia: .asciiz "\nDias com temperatura menor que a média: "
maiormedia: .asciiz "\nDias com temperatura maior que a média: "
...
compara:
	# Variáveis temporárias:
		# $t0 = i (linha)
		# $t1 = j (coluna)
		# $t2 = recebe índice($t0, $t1, $s1) e diz onde guardar o inteiro
		# $t9 = guarda retorno para main
	...
	loop_compara:
		li $t1, 0
		addi $t0, $t0, 1 # Incrementa i
		beq $t0, $s3, fim_compara
		
		# Argumentos para índice
		move $a0, $t0 # i
		move $a3, $s1 # Endereço base da matriz
		jal indice
		move $t2, $v0 # Recebe endereço da posição M[i][j]
		lw $t2, ($t2)
		...
		add $t5, $t5, $t2
		j loop_compara # Se $t1 = número de colunas, incrementa i
		nop
	fim_compara:
		div $t5, $t5, $s3
		move $ra, $t9
...
compara2:
	...
	loop_compara2:
		...
		j loop_compara2 # Se $t1 = número de colunas, incrementa i
		nop
	fim_compara2:
